...
async function getDrinkList(ingredientsArray) {
  ...
  const getMapFromArray = (data) =>
    data.reduce((acc, item) => {
      acc[item.strDrink] = { thumbnail: item.strDrinkThumb, id: item.idDrink };
      return acc;
    }, {});
  if (Array.isArray(fullResult.drinks)) {
    const formattedResult = getMapFromArray(fullResult.drinks);
